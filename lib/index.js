...
//       else resolve()
//     }
//     if (isNPM7OrNewer()) npm.load(callback)
//     else npm.load({ loaded: false }, callback)
//     /* c8 ignore end */
//   })
// }
...
function getPkgInfo(ref) {
  return new Promise((resolve, reject) => {
    ...
    if (id) resolve(pkgCache[id])
      ...
      // npm.commands.v([ref], (err, data, x) => {
      //   if (err) reject(err)
      //   else {
      //     const versions = keys(data)
      //     let { name, version, dist, dependencies = {}, peerDependencies, main, module } = data[versions[versions.length - 1]]
      //     Object.assign(dependencies, peerDependencies)
      //     const pkg = pkgCache[`${name}@${version}`]
      //     if (pkg) resolve(pkg)
      //     else {
      //       const { tarball, rawSize } = dist
      //       module = normalize(module || main || 'index.js')
      //       resolve({ name, version, tarball, rawSize, dependencies, module })
      //     }
      //   }
      // })
...
async function getDepPkgInfos(refs, pkgs, deps) {
  const allDeps = []
  for (let pending = keys(deps); pending.length > 0; pending = pending.slice(concurrency))
  ...
  for (let pending = allDeps; pending.length > 0; pending = pending.slice(concurrency))
    await Promise.all(pending.slice(0, concurrency).map(({ deps }) => getDepPkgInfos(refs, pkgs, deps)))
}
...
//   let { tarURL, tarSize } = pkg
//   if (!tarSize) {
//     const { headers } = await makeRequest('HEAD', tarURL)
//     pkg.tarSize = +headers['content-length']
//   }
// }
...
async function assessBundle(pkg) {
  try {
    ...
    log(`failed ${err.toString().split(/\r?\n/)[1]}`, 'err')
  }
}
...
async function estimatePkgSize(ref) {
  enterLogScope('init')
  log(`inspecting ${ref}`)
  const pkg = await getPkgInfo(ref)
  let { name, version, deps, bundleSize, minifiedSize } = pkg
  const id = `${name}@${version}`
  ...
  for (let pending = data; pending.length > 0; pending = pending.slice(concurrency))
    await Promise.all(pending.slice(0, concurrency).map(pkg => analyseTar(pkg, id)))
  ...
  if (!bundleSize) {
    ...
    ({ bundleSize, miniSize } = pkg)
  }
  return { name, version, tarSize, rawSize, bundleSize, miniSize, depCount }
}

module.exports = async function estimatePkgSizes(refs, { progress, concurrency: c, verbose: v } = {}) {
  concurrency = c || 20
  verbose = v
  disableOut()
  try {
    ...
    for (const ref of refs) {
      const pkg = await estimatePkgSize(ref)
      pkgs.push(pkg)
